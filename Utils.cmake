...
if(NOT SOURCE_MAIN_DIR)
    set(SOURCE_MAIN_DIR src/main)
endif()

if(NOT SOURCE_TEST_DIR)
    set(SOURCE_TEST_DIR src/test)
endif()

if(NOT SOURCE_MAIN_CPP_DIR)
    set(SOURCE_MAIN_CPP_DIR "${SOURCE_MAIN_DIR}/cpp")
endif()

if(NOT SOURCE_TEST_CPP_DIR)
    set(SOURCE_TEST_CPP_DIR "${SOURCE_TEST_DIR}/cpp")
endif()

foreach(p LIB STATIC_LIB BIN INCLUDE CMAKE)
...
macro(set_source_files VARIABLES_PREFIX)
    set(_MULTI_VALUE_ARGS SOURCES HEADERS OBJECTS GENERATED_HEADERS)
...
function(create_module)
    ...
    set(_MULTI_VALUE_ARGS SOURCES HEADERS OBJECTS)
    cmake_parse_arguments(CREATE_MODULE "${_OPTIONS}" "${_ONE_VALUE_ARGS}" "${_MULTI_VALUE_ARGS}" ${ARGN})

    set(CREATE_MODULE_TARGET_OBJECTS)

    foreach(OBJECT ${CREATE_MODULE_OBJECTS})
        list(APPEND CREATE_MODULE_TARGET_OBJECTS $<TARGET_OBJECTS:${OBJECT}>)
    endforeach()

    if(${CREATE_MODULE_COMPILATION_UNIT} STREQUAL "EXECUTABLE")
        add_executable(${CREATE_MODULE_NAME} ${CREATE_MODULE_SOURCES} ${CREATE_MODULE_HEADERS} ${CREATE_MODULE_TARGET_OBJECTS})
    elseif(${CREATE_MODULE_COMPILATION_UNIT} STREQUAL "SHARED_LIBRARY")
        add_library(${CREATE_MODULE_NAME} SHARED ${CREATE_MODULE_SOURCES} ${CREATE_MODULE_HEADERS} ${CREATE_MODULE_TARGET_OBJECTS})
    elseif(${CREATE_MODULE_COMPILATION_UNIT} STREQUAL "STATIC_LIBRARY")
        add_library(${CREATE_MODULE_NAME} STATIC ${CREATE_MODULE_SOURCES} ${CREATE_MODULE_HEADERS} ${CREATE_MODULE_TARGET_OBJECTS})
    elseif(${CREATE_MODULE_COMPILATION_UNIT} STREQUAL "OBJECT_LIBRARY")
        add_library(${CREATE_MODULE_NAME} OBJECT ${CREATE_MODULE_SOURCES} ${CREATE_MODULE_HEADERS} ${CREATE_MODULE_TARGET_OBJECTS})
    elseif(${CREATE_MODULE_COMPILATION_UNIT} STREQUAL "MODULE_LIBRARY")
        add_library(${CREATE_MODULE_NAME} MODULE ${CREATE_MODULE_SOURCES} ${CREATE_MODULE_HEADERS} ${CREATE_MODULE_TARGET_OBJECTS})
    else()
...
function(create_exports)
    ...
    set(CONF_INCLUDE_DIRS "${PROJECT_SOURCE_DIR}/${SOURCE_MAIN_CPP_DIR}" "${PROJECT_BINARY_DIR}/${SOURCE_MAIN_CPP_DIR}")
